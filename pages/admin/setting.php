...
// Get disk space
// [FIX] Tambahkan pengecekan function_exists dan nilai return false untuk hosting yang membatasi fungsi ini
...
// Pastikan nilai numerik (handle jika return false)
...
// Get database size
// [FIX] Gunakan nama database hosting Anda secara eksplisit
$db_name = 'if0_40584396_db_presensikampus';

$q_db_name = @mysqli_query($conn, "SELECT DATABASE()");
if ($q_db_name && $row_db_name = mysqli_fetch_row($q_db_name)) {
    $db_name = $row_db_name[0];
}

$db_size_mb = 0;
if ($db_size_query = @mysqli_query($conn, "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) AS 'size_mb' FROM information_schema.tables WHERE table_schema = '$db_name'")) {
...
.welcome-banner-setting::before {
    ...
    animation: pulse-glow-setting 4s ease-in-out infinite;
}

@keyframes pulse-glow-setting {
    0%, 100% {
        transform: scale(1);
        opacity: 0.5;
    }
...
.welcome-banner-setting .banner-subtitle {
    font-size: 16px;
    opacity: 0.95;
    position: relative;
    z-index: 1;
...
.welcome-banner-setting .banner-icon {
    width: 60px;
    height: 60px;
...
.welcome-banner-setting .banner-badge {
...
/* Dark Mode Support */
[data-theme="dark"] .welcome-banner-setting {
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 col-lg-10">
                <!-- Welcome Banner -->
                <div class="welcome-banner-setting mb-4">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                        <div>
                            <div class="d-flex align-items-center gap-3 mb-2">
                                ...
                                <div>
                                    <h1 class="mb-1"><?= $greeting ?>, Admin!</h1>
                                    <p class="banner-subtitle mb-0">Kelola konfigurasi aplikasi, preferensi sistem, dan pengaturan lainnya</p>
                                </div>
                            </div>
                            <span class="banner-badge">
                                <i class="fas fa-sliders-h me-1"></i>Pengaturan Sistem
                            </span>
                        </div>
                    </div>
                </div>
                                                    ...
                                                    <div class="progress-modern">
                                                        <div class="progress-header-modern">
                                                            ...
                                                            <span class="progress-value-modern">
                                                                <?php if ($disk_total > 0): ?>
                                                                    <?= round($disk_used/1073741824, 2) . ' GB / ' . round($disk_total/1073741824, 2) . ' GB' ?>
                                                                <?php else: ?>
                                                                    <span class="text-muted" style="font-size: 0.9em; font-weight: normal;">Tidak Terdeteksi (Batasan Hosting)</span>
                                                                <?php endif; ?>
                                                            </span>
                                                        </div>
                                                        <div class="progress-bar-modern">
