...
if ($view == 'kelas') {
    ...
    $sql = "SELECT
        ...
        SUM(CASE WHEN m.nim IS NOT NULL AND j.id IS NOT NULL AND (j.sesi = 0 OR j.sesi = m.sesi) AND (p.status = 'alpha' OR ((j.tanggal < CURDATE() OR (j.tanggal = CURDATE() AND ADDTIME(j.jam_mulai, SEC_TO_TIME(30 * 60)) < CURTIME())) AND (p.status IS NULL OR p.status NOT IN ('hadir', 'izin', 'sakit', 'alpha')))) THEN 1 ELSE 0 END) as alpha
        FROM kelas k
    ...
    $sql = "SELECT
        ...
        SUM(CASE WHEN p.status = 'izin' THEN 1 ELSE 0 END) as izin,
        SUM(CASE WHEN p.status = 'sakit' THEN 1 ELSE 0 END) as sakit,
        SUM(CASE WHEN m.nim IS NOT NULL AND (j.sesi = 0 OR j.sesi = m.sesi) AND (p.status = 'alpha' OR ((j.tanggal < CURDATE() OR (j.tanggal = CURDATE() AND j.jam_selesai < CURTIME())) AND (p.status IS NULL OR p.status NOT IN ('hadir', 'izin', 'sakit', 'alpha')))) THEN 1 ELSE 0 END) as alpha
        FROM jadwal j
