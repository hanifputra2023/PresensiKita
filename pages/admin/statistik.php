...
if ($view == 'kelas') {
    ...
    $sql = "SELECT
        ...
        SUM(CASE WHEN m.nim IS NOT NULL AND j.id IS NOT NULL AND (p.status = 'alpha' OR ((j.tanggal < CURDATE() OR (j.tanggal = CURDATE() AND ADDTIME(j.jam_mulai, SEC_TO_TIME(30 * 60)) < CURTIME())) AND (p.status IS NULL OR p.status NOT IN ('hadir', 'izin', 'sakit', 'alpha')))) THEN 1 ELSE 0 END) as alpha
        FROM kelas k
